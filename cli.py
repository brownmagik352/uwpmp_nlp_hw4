...
def get_parser():
    ...
    parser.add_argument(
        '--squad_dir', default=os.path.join('.', 'data', 'squad'))
    parser.add_argument(
        '--glove_dir', default=os.path.join('.', 'data', 'glove'))
...
def auto(config, data_type):
    ...
    config.squad_path = os.path.join(
        config.squad_dir, "{}_small.json".format(data_type))
    config.glove_path = os.path.join(config.glove_dir, "glove.6B.50d.txt")
    config.metadata_path = os.path.join(
        config.out_dir, "metadata_{}.json".format(data_type))
    config.data_path = os.path.join(
        config.out_dir, "data_{}.json".format(data_type))
